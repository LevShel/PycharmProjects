...
for goods_key, goods_value in goods.items():
    total_price = 0
    pcs = 0
    for i in range(len(store[goods[goods_key]])):
        pcs += store[goods[goods_key]][i]['quantity']
        total_price += (store[goods[goods_key]][i]['quantity'] * store[goods[goods_key]][i]['price'])
    print(f'{goods_key} - {pcs} pcs., '
          f'total price: {total_price} rub.')
